function eps_chi_chi_mu = epsilon_bianisotropy_mu_block(pdir, biantype, mutype, lambda2)
...
for i = 1:nx
    for j = 1:ny
        for k = 1:nz
            ...
            [DE1,DM1,DB1] = eblock(i,j,k); %epsilon, mu, biansiotropy
...
BoutE = [Rxm*Ryp*Rzp*Bexx, Rxp*Rym*Rzp*Bexy, Rxp*Ryp*Rzm*Bexz;
         Rxm*Ryp*Rzp*Beyx, Rxp*Rym*Rzp*Beyy, Rxp*Ryp*Rzm*Beyz;
         Rxm*Ryp*Rzp*Bezx, Rxp*Rym*Rzp*Bezy, Rxp*Ryp*Rzm*Bezz];
...
%     Qyy = (3*dir2*dir2 * (1 + 2*G/(3*deps(i+1/2,j,k))) -1)/2;
%     Qyz = (3*dir2*dir3 * (1 + 2*G/(3*deps(i+1/2,j,k))))/2;
%     Qzz = (3*dir3*dir3 * (1 + 2*G/(3*deps(i+1/2,j,k))) -1)/2;
...
%     [TQ,DQ] = eig(Qten);
%     [TQ,DQ] = sortem(TQ,DQ);
%    DQ(1,1) = DQ(1,1) * (1 + G/(DQ(1,1)*deps(i+1/2,j,k)));
%    DQ(2,2) = DQ(2,2) * (1 + G/(DQ(2,2)*deps(i+1/2,j,k)));
...
%     [TQ,DQ] = eig(Qten);
%     [TQ,DQ] = sortem(TQ,DQ);
%        DQ(1,1) = DQ(1,1) * (1 + G/(DQ(1,1)*deps(i,j+1/2,k)));
%        DQ(2,2) = DQ(2,2) * (1 + G/(DQ(2,2)*deps(i,j+1/2,k)));
